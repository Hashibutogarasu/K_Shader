...
void main() {
    ...
    gl_Position.x += sin((frameTimeCounter * pi / (13.0 * speed)) + (gl_Position.x + d0)*0.9 + (gl_Position.z + d1)*0.9);
    gl_Position.y += sin((frameTimeCounter * pi / (15.0 * speed)) + (gl_Position.z + d2) + (gl_Position.x + d3));
    gl_Position.y -= 0.04;
